  ...
  return (
    <Card className="bg-background/60 dark:bg-default-100/50" shadow="md">
      <CardHeader className="flex justify-between items-center px-6 pb-0">
        <div className="font-semibold text-xl">Workout Summary</div>
        <Tabs
      ...
      <CardBody className="px-6">
        <div className="space-y-1 text-sm">
          <div className="flex justify-between py-1 pb-2 border-b light:border-black/10 dark:border-white/10">
            ...
            <span>
              {(paxData?.[selected as keyof typeof paxData]?.totalEvents ?? 0) > 0
                ? paxData?.[selected as keyof typeof paxData]?.totalEvents
                : "Has not posted yet"}
            </span>
          </div>
          <div
          ...
          >
            ...
            <span>
              {(paxData?.[selected as keyof typeof paxData]?.totalQ ?? 0) > 0
          ...
          >
            ...
            <span>
              {eventsResult?.uniquePax?.most_attended_user_event_count ?? 0} BDs
              with{" "}
              <Link
                className="text-sm"
                color="warning"
          ...
          {(paxData?.[selected as keyof typeof paxData]?.totalEvents ?? 0) > 0 && (
            <>
              ...
              >
                ...
                <span>
                  {paxData?.[selected as keyof typeof paxData]?.firstBD ?? 0} at{" "}
                  <Link
                    className="text-sm"
                    color="warning"
                    href={`/stats/ao/${paxData?.[selected as keyof typeof paxData]?.firstBDLocation?.id}`}
                  >
                    {paxData?.[selected as keyof typeof paxData]?.firstBDLocation?.name}
                  </Link>
                </span>
              </div>
              <div className="flex justify-between py-1 pb-2 border-b light:border-black/10 dark:border-white/10">
                ...
                <span>
                  {paxData?.[selected as keyof typeof paxData]?.lastBD ?? 0} at{" "}
                  <Link
                    className="text-sm"
                    color="warning"
                    href={`/stats/ao/${paxData?.[selected as keyof typeof paxData]?.lastBDLocation?.id}`}
                  >
                    {paxData?.[selected as keyof typeof paxData]?.lastBDLocation?.name}
                  </Link>
                </span>
              </div>
              <div
          ...
          {(paxData?.[selected as keyof typeof paxData]?.totalQ ?? 0) > 0 && (
            <>
              <div className="flex justify-between py-1 pb-2 border-b light:border-black/10 dark:border-white/10">
                ...
                <span>
                  {paxData?.[selected as keyof typeof paxData]?.firstQ ?? 0} at{" "}
                  <Link
                    className="text-sm"
                    color="warning"
                    href={`/stats/ao/${paxData?.[selected as keyof typeof paxData]?.firstQLocation?.id}`}
                  >
                    {paxData?.[selected as keyof typeof paxData]?.firstQLocation?.name}
                  </Link>
                </span>
              </div>
              <div className="flex justify-between py-1 pb-2 border-b light:border-black/10 dark:border-white/10">
                ...
                <span>
                  {paxData?.[selected as keyof typeof paxData]?.lastQ ?? 0} at{" "}
                  <Link
                    className="text-sm"
                    color="warning"
                    href={`/stats/ao/${paxData?.[selected as keyof typeof paxData]?.lastQLocation?.id}`}
                  >
                    {paxData?.[selected as keyof typeof paxData]?.lastQLocation?.name}
                  </Link>
                </span>
              </div>
              <div className={`flex justify-between py-1 pb-2`}>
                ...
                <span>
                  {eventsResult?.uniquePax?.unique_attendees_when_q ?? 0} PAX
                </span>
              </div>
            </>
          )}
