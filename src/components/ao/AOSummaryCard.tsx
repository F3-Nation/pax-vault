...
export function AOSummaryCard({ summary }: { summary: AOSummary }) {
  return (
    <Card className="bg-background/60 dark:bg-default-100/50" shadow="md">
      ...
      <CardBody className="px-6">
        <div className="flex justify-between py-1 pb-2 border-b light:border-black/10 dark:border-white/10">
          ...
          <span>
            {summary?.first_start_time
              ? formatDate(summary.first_start_time, "M D Y")
              : "Unknown Start Date"}
          </span>
        </div>
        <div className="flex justify-between py-1 pb-2 border-b light:border-black/10 dark:border-white/10">
          ...
          <span>
            {summary?.total_workouts
              ? formatNumber(summary.total_workouts)
              : "Unknown"}{" "}
            Workouts
          </span>
        </div>
        ...
        <div className="flex justify-between py-1 pb-2 border-b light:border-black/10 dark:border-white/10">
          ...
          <span>
            {summary?.total_fngs ? formatNumber(summary.total_fngs) : "Unknown"}{" "}
            FNGs
          </span>
        </div>
        <div className="flex justify-between py-1 pb-2 border-b light:border-black/10 dark:border-white/10">
          ...
          <span>
            {summary?.avg_pax_count
              ? formatNumber(summary.avg_pax_count, 2)
              : "Unknown"}{" "}
            PAX
          </span>
        </div>
        <div className="flex justify-between py-1 pb-2">
          ...
          <span>
            {summary?.peak_pax_count
              ? formatNumber(summary.peak_pax_count)
              : "Unknown"}{" "}
            PAX
          </span>
        </div>
