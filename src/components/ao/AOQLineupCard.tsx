...
export function AOQLineupCard({ lineup }: { lineup: AOQLineup[] }) {
            ...
            lineup.map((item, index) => (
              ...
              >
                <div className="flex flex-col">
                  ...
                  >
                    <div className="text-primary">{item.location_name}</div>
                  </Link>
                  <span className="text-left text-sm text-default-400">
                    {formatDate(item.start_date, "M D Y")} @{" "}
                    {formatTime(item.start_time)} [{item.event_types}]
                  </span>
                </div>
                <div>
                  {item.q_who ? (
