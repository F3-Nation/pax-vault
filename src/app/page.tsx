...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-background to-default-50 px-4">
      <Card className="w-full max-w-3xl bg-background/80 dark:bg-default-100/60" shadow="lg">
        <CardHeader className="flex flex-col gap-3">
          ...
          <p className="text-sm text-foreground/70 max-w-xl">
            A focused stats and history vault for F3 PAX, AOs, and Regions. Track workouts,
            Qs, attendance, and leadership impact in one place — built for Site Qs, Nantan,
            and data nerds who actually care about the numbers.
          </p>
        </CardHeader>

        <CardBody className="space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            <Card className="bg-default-50/80 dark:bg-default-100/80" shadow="none">
              <CardBody className="gap-2">
                ...
                <p className="text-xs text-foreground/70">
                  See streaks, attendance, Q history, and region impact per PAX with
                  clean, drill-down views.
                </p>
              </CardBody>
            </Card>
            ...
            <Card className="bg-default-50/80 dark:bg-default-100/80" shadow="none">
              <CardBody className="gap-2">
                ...
                <p className="text-xs text-foreground/70">
                  Understand AO health, Q coverage, and growth trends across your region at
                  a glance.
                </p>
              </CardBody>
            </Card>
            ...
            <Card className="bg-default-50/80 dark:bg-default-100/80" shadow="none">
              <CardBody className="gap-2">
                ...
                <p className="text-xs text-foreground/70">
                  Designed around F3 language and workflows — not generic fitness tracking.
                </p>
              </CardBody>
            </Card>
          </div>
          ...
          <div className="flex flex-col items-center gap-3 pt-2">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:justify-center">
              <Link href={`/stats/pax/${process.env.SAMPLE_PAX ?? ""}`} className="flex-1 sm:flex-none">
                <Button
                  fullWidth
                  variant="bordered"
                  color="secondary"
              ...
              <Link href={`/stats/region/${process.env.SAMPLE_REGION ?? ""}`} className="flex-1 sm:flex-none">
                <Button
                  fullWidth
                  variant="bordered"
                  color="primary"
        ...
        <CardFooter className="flex flex-col sm:flex-row items-center justify-between gap-2 border-t border-default-100/60">
          <p className="text-[11px] text-foreground/50">
            Built for F3 regions that want real visibility into their PAX and AOs.
          </p>
          <p className="text-[11px] text-foreground/50">
