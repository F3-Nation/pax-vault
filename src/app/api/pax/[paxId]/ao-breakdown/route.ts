...
export async function GET(
  ...
  const aoBreakdown = await getAOBreakdown(paxId, opts);

  if (!aoBreakdown) {
    return NextResponse.json({ error: "Pax not found" }, { status: 404 });
  }

  return NextResponse.json(aoBreakdown, { status: 200 });
}
