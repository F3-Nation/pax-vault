...
export async function GET(
  ...
  const leaders = await getLeaders(aoId, opts);

  if (!leaders) {
    return NextResponse.json(
      { error: "AO leaders not found" },
      { status: 404 },
    );
  }
