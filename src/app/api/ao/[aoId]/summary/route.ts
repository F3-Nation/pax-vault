...
export async function GET(
  ...
  const summary = await getSummary(aoId, opts);

  if (!summary) {
    return NextResponse.json(
      { error: "AO summary not found" },
      { status: 404 },
    );
  }
