...
export function getSummary(
  ...
  const first_event_date =
    data.events.length > 0
      ? formatDate(new Date(data.events[0].event_date))
      : null;
  const first_event_ao_id =
    data.events.length > 0 ? data.events[0].event_instance_id : null;
  const first_event_ao_name =
    data.events.length > 0 ? data.events[0].ao_name : null;
  const last_event_date =
    data.events.length > 0
      ? formatDate(new Date(data.events[data.events.length - 1].event_date))
      : null;
  const last_event_ao_id =
    data.events.length > 0
      ? data.events[data.events.length - 1].event_instance_id
      : null;
  const last_event_ao_name =
    data.events.length > 0 ? data.events[data.events.length - 1].ao_name : null;
  ...
  if (bestie_user_id !== null) {
    for (const event of data.events) {
      const match = event.attendance.find(
        (att) => att.user_id === bestie_user_id,
      );
      if (match) {
...
export function getPaxCharting(data: PaxData): PaxInsights {
  ...
  if (paxDataArray.length > 0) {
    ...
    const firstDate = paxDataArray.reduce(
      (min, d) => {
